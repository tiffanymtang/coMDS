...
if (is.null(opt$results_dir)) {
  RESULTS_DIR <- here::here("results", "swiss_roll")
} else {
  RESULTS_DIR <- file.path(opt$results_dir, "swiss_roll")
}
...
dgp_out <- swiss_roll_dgp(t_max = 3.4 * pi) # good example
saveRDS(
  dgp_out,
  file = file.path(RESULTS_DIR, "Swiss_roll.rds")
)
